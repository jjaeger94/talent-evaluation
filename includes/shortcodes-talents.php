...
function render_test_start() {
     ...
     if (isset($_GET['tid'], $_GET['jid'], $_GET['key'])) {
          $tid = intval($_GET['tid']);
          $jid = intval($_GET['jid']);
          ...
          if (commitment_hash($jid.$tid) === $key) {
               ...
               $questions = get_questions_by_test_id($tid); // Hier müsstest du die entsprechende Funktion implementieren
...
function render_commitment_test(){
     if(isset($_GET['uid'], $_GET['key'])){
          ...
          if(commitment_hash($uid) == $key){
               ...
               $link = esc_url( home_url( '/test-starten/?tid=' . $test->ID . '&jid=' . $jid ));
               // Füge die Application ID hinzu, wenn verfügbar
               if (isset($aid)) {
                    $link .= '&aid=' . $aid;
               }
               // Füge den Hash-Key hinzu
               $link .= '&key=' . commitment_hash($jid . $test->ID);
               ob_start();
